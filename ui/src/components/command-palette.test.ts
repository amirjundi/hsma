...
describe("CommandPalette lifecycle", () => {
  ...
  it.each([
    ...
    "shows Ask HSMA only when availability is $available",
    async ({ available, expectedCount }) => {
      const { gateway } = createGateway(true);
      const { palette } = await mountPalette(
        createContext(
      ...
      expect(findPaletteOption(palette, "Ask HSMA", true) ? 1 : 0).toBe(expectedCount);
    },
  );

  it("opens Ask HSMA from its palette action", async () => {
    const { gateway } = createGateway(true);
    const { palette } = await mountPalette(
      createContext(
        gateway,
    ...
    try {
      findPaletteOption(palette, "Ask HSMA", true)?.click();
    } finally {
