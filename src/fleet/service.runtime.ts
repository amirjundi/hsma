        ...
        operation: async (checkpoint) => {
          ...
          if (result.started) {
            try {
              ...
              throw new Error(
                `Fleet cell ${tenantId} was created but did not become healthy within 60s; inspect it with \`hsma fleet status ${tenantId}\` or \`hsma fleet logs ${tenantId}\`, or remove it with \`hsma fleet rm ${tenantId} --force\`.`,
                { cause: error },
              );
            }
          }
