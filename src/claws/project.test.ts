...
describe("Claw projects", () => {
  ...
  it("creates a minimal project that validates through the canonical reader", async () => {
    ...
    if (validated.ok) {
      ...
      expect(validated.claw.clawMarkdownBody?.toString()).toContain("purpose-built HSMA agent");
    }
  });
  ...
  it("preserves the canonical metadata-selected HSMA profile path", async () => {
