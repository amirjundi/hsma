...
describe("HSMA profile reader", () => {
  it("loads and integrity-binds the conventional profile", async () => {
    ...
    if (!first.ok) {
      throw new Error("expected HSMA profile to parse");
    }
    ...
    if (!second.ok) {
      throw new Error("expected changed HSMA profile to parse");
    }
