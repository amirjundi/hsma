...
describe("resolveEffectiveAgentRuntime", () => {
  ...
  it("resolves residual auto to HSMA when no plugin harness is registered", () => {
    expect(
      resolveEffectiveAgentRuntime({
        cfg: {
          models: {
  ...
  it("keeps an authored custom route on HSMA before registered harness selection", () => {
  ...
  it("lets an explicit HSMA override replace configured Codex policy", () => {
    expect(
      resolveEffectiveAgentRuntime({
