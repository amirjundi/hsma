...
describe("lockWorktreeForProcess", () => {
  ...
  it("reclaims a lock left behind by a dead HSMA process", async () => {
  ...
  it("keeps a lock held by a live HSMA process", async () => {
    const record = await setupWorktree();
  ...
  it("keeps a foreign lock that HSMA does not own", async () => {
    const record = await setupWorktree();
