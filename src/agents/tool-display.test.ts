...
describe("tool display details", () => {
  ...
  it("preserves JS numeric literals in tool-search call args", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: 'return await openclaw.tools.call("web_search", { query: "HSMA", count: 1e3, limit: +3, threshold: .5 });',
      })?.displayArgs,
    ).toEqual({
      query: "HSMA",
      count: 1000,
  ...
  it("formats web_search query with quotes", () => {
    const detail = formatToolDetail(
      resolveToolDisplay({
        name: "web_search",
        args: { query: "HSMA docs", count: 3 },
      }),
    );

    expect(detail).toBe('for "HSMA docs" (top 3)');
  });
    ...
    expect(
      formatToolDetail(
        resolveToolDisplay({
          name: "web_search",
          args: {
            objective: "Find the HSMA repository on GitHub",
      ...
      'for "Find the HSMA repository on GitHub", "openclaw github", "openclaw repository" (top 5)',
    );
  });
  ...
  it("omits bash and exec names from compact tool summaries", () => {
    ...
    expect(
      formatToolSummary(
        resolveToolDisplay({
          name: "web_search",
          args: { query: "HSMA docs" },
        }),
      ),
    ).toBe('🔎 Web Search: for "HSMA docs"');
  });
