...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes an HSMA control section", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
      toolNames: ["gateway"],
    });

    expect(prompt).toContain("## HSMA Control");
  ...
  it("keeps the HSMA empty-tool fallback capability-only", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
      toolNames: [],
    });

    expect(prompt).toContain("active runtime provides the available HSMA tools directly");
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain(
      "HSMA behavior questions: docs first via `Read`/local search. AGENTS/project/workspace/profile/memory = instructions/user memory, not product design truth.",
    );
  });
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Local subagent list/status tools cover HSMA runtime=subagent only");
