...
describe("OpenAI runtime routing policy", () => {
  ...
  ])("keeps %s values on the HSMA runtime", (_label, params) => {
    const config = {
      agents: {
        defaults: {
          models: {
  ...
  it("fails closed to HSMA when the provider artifact is unavailable", () => {
  ...
  it("honors the deprecated whole-agent HSMA runtime opt-out", () => {
    const config = {
      agents: {
  ...
  it("keeps per-model Codex policy above the whole-agent HSMA opt-out", () => {
    const config = {
      agents: {
        defaults: {
  ...
  it("keeps per-model auto policy above the whole-agent HSMA opt-out", () => {
    const config = {
      agents: {
        defaults: {
          agentRuntime: { id: "openclaw" },
  ...
  it("keeps explicit HSMA plus Codex auth profile under the unified OpenAI provider", () => {
  ...
  it("keeps explicit OpenAI HSMA API-key auth order ahead of Codex backups", () => {
    const config = {
      auth: {
        order: {
  ...
  it("does not route custom OpenAI-compatible HSMA configs through Codex auth order", () => {
    const config = {
      models: {
        providers: {
          openai: {
