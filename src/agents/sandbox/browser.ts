      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `hsma sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `hsma sandbox recreate --browser --agent ${agentId}`;
          }
          return "hsma sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
