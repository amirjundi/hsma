...
describe("resolveSandboxedMediaSource", () => {
  ...
  it("rejects symlinked HSMA tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked HSMA tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
  ...
  it("rejects symlinked HSMA tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOpenClawTmp(
