...
function buildMissingProviderModelRegistrationHint(params: {
  ...
  if (normalizeProviderId(params.provider) === "openai-codex") {
    return `"openai-codex" is a legacy provider ID. Run \`hsma doctor --fix\` to migrate legacy model and provider config to the current OpenAI format. If the provider has no authenticated profile, run \`hsma models status\` to check provider auth and re-authenticate if needed. See https://docs.openclaw.ai/concepts/model-providers.`;
  }
