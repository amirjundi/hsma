...
describe("compactEmbeddedAgentSessionDirect hooks", () => {
  ...
  it("keeps model-locked HSMA compaction on its exact model without fallbacks", async () => {
  ...
  it("preserves direct OpenAI API-key compaction when HSMA runtime is active", async () => {
    resolveAgentHarnessPolicyMock.mockReturnValue({ runtime: "openclaw" });
...
describe("compactEmbeddedAgentSession hooks (ownsCompaction engine)", () => {
  ...
  it("preserves concrete HSMA pins over explicit Codex policy for queued compaction", async () => {
    resolveAgentHarnessPolicyMock.mockReturnValue({
      runtime: "codex",
      runtimeSource: "model",
    } as never);
