...
describe("system agent setup-flow operations", () => {
  ...
  it("prints one-shot setup pointers", async () => {
    ...
    expect(output).toContain("on the machine running HSMA");
  });
  ...
  it("routes one-shot model setup through the verified HSMA flow", async () => {
    const { runtime, lines } = createSystemAgentTestRuntime();

    const result = await executeSystemAgentOperation({ kind: "model-setup" }, runtime);

    expect(result.applied).toBe(false);
    expect(lines.join("\n")).toContain("Run `openclaw onboard` on the machine running HSMA");
    expect(lines.join("\n")).toContain("Stop the HSMA host");
