...
export async function verifySetupInference(
  ...
  if (!configuredRoute || !verifiedBinding) {
    return {
      ...
      status: "unknown",
      error:
        "The successful inference run did not report an exact execution binding. Retry setup before starting HSMA.",
    };
  }
...
export async function completeSetupInference(params: {
  ...
  const snapshot = await readSnapshot();
  if (!snapshot.exists) {
    return { ok: false, status: "unavailable", error: "No HSMA config exists." };
  }
  if (!snapshot.valid) {
