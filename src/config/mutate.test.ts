...
describe("config mutate helpers", () => {
  ...
  it.each(["EACCES", "EPERM", "EROFS"] as const)(
    ...
    async (code) => {
      ...
      await expect(replaceConfigFile({ snapshot, nextConfig: {} })).rejects.toMatchObject({
        name: "Error",
        message: `HSMA cannot write to the config directory ${configDir}. Fix its ownership or permissions, then try again. Underlying error: ${failure.message}`,
        cause: failure,
      });
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await expect(replaceConfigFile({ snapshot, nextConfig: {} })).rejects.toMatchObject({
        message: `HSMA cannot write to the config directory ${configuredDir}. Fix its ownership or permissions, then try again. Underlying error: ${failure.message}`,
        cause: failure,
      });
    },
  );
  ...
  it("refuses mutate writes in Nix mode before touching disk", async () => {
    ...
    ).rejects.toThrow("HSMA Nix overview: https://docs.openclaw.ai/install/nix");
