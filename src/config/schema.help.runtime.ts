...
export const RUNTIME_FIELD_HELP: Record<string, string> = {
  ...
  "browser.allowSystemProfileImport":
    "Allows macOS hosts to import cookies from a local Chrome-family system profile into a managed HSMA browser profile. Disable this to prevent browser profile cookie import and its macOS Keychain consent prompt.",
  "browser.cdpUrl":
  ...
  "tools.toolSearch":
    "Compact large HSMA, MCP, and client tool catalogs. Set to true for the default code bridge or use the object form to choose structured controls or a compact visible tool directory.",
  "tools.toolSearch.enabled":
    "Enables Tool Search. When on, HSMA hides large tool catalogs behind `tool_search_code` or structured search/describe/call tools during embedded runtime runs.",
  ...
  "tools.codeMode":
    "Generic HSMA code mode. When enabled, agent runs expose only `exec` and `wait` to the model and hide normal tools behind a QuickJS-WASI catalog bridge.",
  ...
  "gateway.push.apns.relay":
    "External relay settings for relay-backed APNs sends. The gateway uses the hosted HSMA relay by default, or this custom relay for push.test, wake nudges, and reconnect wakes after a paired official iOS build publishes a relay-backed registration.",
  ...
  "gateway.nodes.pairing.sshVerify":
    "SSH-verified auto-approval for first-time node-role device pairing (default: enabled). The gateway SSHes back to the pairing host (BatchMode, strict host keys) and approves only when the remote `hsma node identity` output matches the pending device key. Set false to disable SSH verification (independent of autoApproveCidrs, which stays active); for manual-only pairing also unset autoApproveCidrs. Pass an object to override user/identity/timeoutMs/cidrs.",
  ...NODE_CAPABILITY_FIELD_HELP,
  "gateway.nodes.commands.allow":
    "Extra node.invoke commands to allow beyond the gateway defaults (array of command strings). Enabling dangerous commands here is a security-sensitive override and is flagged by `hsma security audit`.",
  ...
  "nodeHost.workerRuns":
    "Opt in to full HSMA worker session hosting from Gateway-managed bundles. Disabled by default.",
  ...
  "nodeHost.browserProxy.allowProfiles":
    "Optional allowlist of browser profile names exposed through node proxy routing. Leave empty to preserve the default full profile surface, including profile create/delete routes. When set, HSMA enforces least-privilege profile access and blocks persistent profile create/delete through the proxy.",
  "nodeHost.mcp":
  ...
  "diagnostics.otel.protocol":
    'OTel transport protocol for telemetry export. Only "http/protobuf" is accepted; run "hsma doctor --fix" to repair a persisted legacy "grpc" value or get source-specific manual-edit guidance.',
  ...
  "skills.load.extraDirs":
    "Additional shared skill roots to scan at lowest precedence. Use this for sibling repos or shared skill packs that should be available without copying them into the HSMA workspace.",
