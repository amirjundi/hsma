...
export function resolveFutureConfigActionBlock(
  ...
  return {
    ...
    currentVersion,
    touchedVersion,
    message: `Refusing to ${params.action} because this HSMA binary (${currentVersion}) is older than the config last written by HSMA ${touchedVersion}.`,
    hints: [
      "Run the newer hsma binary on PATH, or reinstall the intended gateway service from the newer install.",
