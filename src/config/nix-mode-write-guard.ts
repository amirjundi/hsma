...
function formatNixModeConfigMutationMessage(params: { configPath?: string } = {}): string {
  return [
    ...
    "Do not run setup, onboarding, hsma update, plugin install/update/uninstall/enable, doctor repair/token-generation, or config set against this file.",
    ...
    `HSMA Nix overview: ${OPENCLAW_NIX_OVERVIEW_URL}`,
  ].join("\n");
}
