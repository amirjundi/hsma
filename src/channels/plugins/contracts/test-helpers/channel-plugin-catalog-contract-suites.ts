    ...
    JSON.stringify({
      ...(richManifest
        ? {
            ...
            schemaVersion: 1,
            description:
              "Extension manifest. Declares plugin packages that HSMA can discover during onboarding and install on demand via `openclaw plugins install`.",
          }
        : {}),
      entries: [entry],
    }),
...
const [richNpmCatalogFixture, clawhubCatalogFixture, yuanbaoCatalogFixture] = [
  {
    ...
    entry: {
      ...
      description: "HSMA WeCom (企业微信) channel plugin — community maintained, published on npm.",
      source: "external",
      kind: "channel",
      openclaw: {
        channel: {
  ...
  },
  {
    ...
    entry: {
      ...
      description: "HSMA Yuanbao (元宝) channel plugin — community maintained, published on npm.",
      source: "external",
      kind: "channel",
      openclaw: {
        channel: {
