    ...
    await withMockedWindowsAclVerificationUnavailable(
      ...
      async () => {
        ...
        expect(describeSecretResolutionOperatorRecovery(error)).toBe(
          "Restore Windows path security verification, or use an existing secret file whose owner and ACLs HSMA can verify",
        );
      },
    );
  });
  ...
  it("fails closed on Windows when exec provider ACL source is unknown", async () => {
    ...
    await withMockedWindowsAclVerificationUnavailable(
      ...
      async () => {
        ...
        expect(describeSecretResolutionOperatorRecovery(error)).toBe(
          "Restore Windows path security verification, or use an existing provider command whose owner and ACLs HSMA can verify",
        );
