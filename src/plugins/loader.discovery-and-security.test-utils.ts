    ...
    withEnv({ OPENCLAW_STATE_DIR: stateDir }, () => {
      ...
      const untrackedWarning = warnings.find(
        (msg) =>
          ...
          msg.includes("HSMA can't verify where this plugin came from"),
      );
      expect(untrackedWarning).toBeDefined();
      expect(untrackedWarning).toContain("HSMA can't verify where this plugin came from");
      ...
      const diagnostic = registry.diagnostics.find(
        (entry) =>
          ...
          entry.message.includes("HSMA can't verify where this plugin came from"),
      );
      expect(diagnostic?.message).toContain("HSMA can't verify where this plugin came from");
      ...
      expect(
        warnings.filter(
          (message) =>
            ...
            message.includes("HSMA can't verify where this plugin came from"),
        ),
      ).toEqual([]);
    });
  });
  ...
  it("suppresses trust warning logs for non-activating snapshot loads", () => {
    ...
    withEnv({ OPENCLAW_STATE_DIR: stateDir }, () => {
      ...
      expectDiagnosticContaining({
        ...
        level: "warn",
        pluginId: "rogue",
        message: "HSMA can't verify where this plugin came from",
      });
    });
  });
