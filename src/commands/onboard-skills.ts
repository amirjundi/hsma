...
export async function setupSkills(
  ...
  if (candidateInstallable.length === 0 && missing.length === 0) {
    await prompter.note(
      [
        ...
        `To inspect available skills, run: ${formatCliCommand("hsma skills list --verbose")}`,
        `To check skill status, run: ${formatCliCommand("hsma skills check")}`,
      ].join("\n"),
  ...
  if (candidateInstallable.length > 0) {
    ...
    for (const target of selectedReadySkills) {
      ...
      // visible through `hsma skills list --verbose`.
      ...
      runtime.log(
        `Tip: run \`${formatCliCommand("hsma doctor")}\` to review skills + requirements.`,
      );
