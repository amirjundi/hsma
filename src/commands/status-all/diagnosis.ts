// Appends the read-only diagnosis section for `hsma status --all`.
  ...
  if (params.portUsage) {
    ...
    if (!portOk) {
      ...
      if (gatewayPidCount > 1) {
        lines.push(
          `  ${muted(`${gatewayPidCount} HSMA gateway processes appear to be listening on port ${params.port}; stop stale gateway processes before trusting channel health.`)}`,
        );
      }
      ...
      lines.push(`  ${muted("Detected HSMA Gateway listener on the configured port.")}`);
    }
  }
  ...
  if (!params.nodeOnlyGateway && params.exporterDiagnostics) {
    if (params.exporterDiagnostics.ok) {
      ...
      emitUnavailableDiagnostics({
        ...
        retry: "hsma gateway stability --type telemetry.exporter",
      });
    }
  }
