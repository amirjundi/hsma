...
describe("cleanupLegacyPluginDependencyState", () => {
  ...
  it("removes configured plugin stage roots outside HSMA roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
  ...
  it("refuses arbitrary explicit plugin stage roots outside HSMA roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
  ...
  it("does not follow symlinked extension roots outside HSMA roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
  ...
  it("refuses legacy roots that resolve outside HSMA roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
    const legacyRuntimeRoot = path.join(stateDir, "plugin-runtime-deps");
    ...
    expect(result.changes).toStrictEqual([]);
    expect(result.warnings).toContain(
      `Skipped legacy plugin dependency state ${legacyRuntimeRoot}: resolved outside HSMA cleanup roots`,
    );
