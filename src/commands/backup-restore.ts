...
const BACKUP_RESTORE_WARNINGS = [
  ...
  "Plugin node_modules are not archived; after activation, run `hsma plugins update <id>` or reinstall with `hsma plugins install <spec> --force`.",
  "Generated plugin-skills links are not archived; after activation, run `hsma skills list` or start an agent session to rebuild them.",
] as const;
...
async function assertTargetOutsideLiveState(targetPath: string): Promise<void> {
  ...
  if (isPathWithin(canonicalTarget, canonicalStateDir)) {
    throw new Error(
      `Backup restore target must be outside the live HSMA state directory: ${targetPath}`,
    );
  }
  ...
  for (const { sourcePath } of agentRoots) {
    if (isPathWithin(canonicalTarget, sourcePath)) {
      throw new Error(
        `Backup restore target must be outside the live HSMA agent directory: ${targetPath}`,
      );
    }
  }
}
...
function formatRestoreResult(result: BackupRestoreResult): string {
  return [
    ...
    "",
    "Activation is explicit: stop the Gateway, move the restored asset tree into place or point OPENCLAW_STATE_DIR at the restored state asset, then run `hsma doctor`.",
  ].join("\n");
}
