...
export async function buildGatewayInstallPlan(params: {
  ...
  if (wrapperPointsAtWindowsTaskScript) {
    params.warn?.(
      `Ignoring ${OPENCLAW_WRAPPER_ENV_KEY} because it points to the Windows task script; using the HSMA gateway entrypoint directly to avoid a recursive gateway.cmd wrapper.`,
    );
  }
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("hsma gateway install")}\` after fixing the error.`;
}
