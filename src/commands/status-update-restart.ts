...
export function formatUpdateRestartStatusValue(
  ...
  if (payload.status === "error") {
    return warn(`failed · ${reason ?? "restart failed"} · run hsma gateway status --deep${age}`);
  }
  ...
  if (payload.status === "skipped") {
    if (reason === CONTROL_PLANE_UPDATE_HANDOFF_STARTED_REASON) {
      ...
      return warn(`handoff running · gateway restart pending · run hsma update status${age}`);
    }
    if (reason === CONTROL_PLANE_UPDATE_RESTART_HEALTH_PENDING_REASON) {
      ...
      return warn(`restart pending health verification · run hsma gateway status --deep${age}`);
    }
...
export function formatUpdateRestartActionLines(
  ...
  if (payload.status === "error") {
    return [
      "Update restart failed; run hsma gateway status --deep.",
      "If the service is down, run hsma gateway restart or hsma gateway install --force.",
    ];
  }
  const reason = readReason(payload);
  if (
    ...
    return [
      "Update restart is still pending; run hsma update status --json for handoff state.",
      "If it stays pending, run hsma gateway status --deep.",
    ];
  }
  return [];
}
