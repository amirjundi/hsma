...
export async function compactDoctorSessionSqliteTarget(
  ...
  if (!stat.isFile()) {
    throw new Error(`HSMA agent database is not a regular file: ${sqlitePath}`);
  }
  if (isOpenClawAgentDatabaseOpen(sqlitePath)) {
    throw new Error(
      `HSMA agent database ${sqlitePath} is already open in this process. Stop HSMA and retry.`,
    );
  }
