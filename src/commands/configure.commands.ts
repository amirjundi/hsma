 ...
 * Mirrors the real `hsma config <sub>` surface so the message only ever
...
const CONFIGURE_NON_TTY_HINT = [
  ...
  `  ${formatCliCommand("hsma config set <path> <value>")}  write a config entry`,
  `  ${formatCliCommand("hsma config get <path>")}          read a config entry`,
  `  ${formatCliCommand("hsma config patch")}              apply a JSON patch`,
  `  ${formatCliCommand("hsma config validate")}           validate configuration`,
].join("\n");
...
export async function configureCommandFromSectionsArg(
  ...
  if (invalid.length > 0) {
    runtime.error(
      `Invalid --section: ${invalid.map((section) => section || '""').join(", ")}. Expected one of: ${CONFIGURE_WIZARD_SECTIONS.join(", ")}. Run ${formatCliCommand("hsma configure")} without --section to use the full wizard.`,
    );
    runtime.exit(1);
    return;
  }

  // Fail closed once at the shared entry: both `hsma configure` and the
  // no-subcommand `hsma config` route here, so a single guard keeps them
