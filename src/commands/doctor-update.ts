...
async function restartRunningGatewayServiceAfterUpdate(
  ...
  try {
    ...
    note("Restarted the running gateway service after updating HSMA.", "Update");
    return true;
  } catch (err) {
...
export async function maybeOfferUpdateBeforeDoctor(params: {
  ...
  if (git === "git") {
    const shouldUpdate = await params.confirm({
      message: "Update HSMA from git before running doctor?",
      initialValue: true,
    });
    if (!shouldUpdate) {
      return { updated: false };
  ...
  if (git === "not-git") {
    note(
      [
        ...
        `Run \`${formatCliCommand("hsma update")}\` to update via your package manager (npm/pnpm), then rerun doctor.`,
      ].join("\n"),
      "Update",
    );
  }
