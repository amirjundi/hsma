...
export function resolveExplicitSessionStorePath(params: {
  ...
  try {
    ...
    if (
      ...
      throw new Error("the SQLite file has application tables but no HSMA schema metadata");
    }
  } catch (error) {
  ...
  if (databaseFailure) {
    throw new Error(
      `Session store target is not a session store: ${displayTarget}. ${formatErrorMessage(databaseFailure.error)}. Pass a legacy store selector or SQLite target reported by hsma sessions or hsma status.`,
    );
  }
  return storePath;
}
