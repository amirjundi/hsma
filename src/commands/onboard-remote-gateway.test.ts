...
describe("runRemoteGatewayInferenceOnboarding", () => {
  it.each([
    ...
    "pins $label across detect, activate, verify, HSMA, and in-process TUI",
    async ({ auth, secret }) => {
  ...
  ])("fails closed on $label before HSMA", async ({ verification, error }) => {
    const localConfig = makeLocalConfig();
    const localConfigBefore = structuredClone(localConfig);
    const methods: string[] = [];
    const callGatewayMock = vi.fn(async (options: CallGatewayCliOptions): Promise<unknown> => {
  ...
  it("treats a cancelled remote HSMA conversation as a pause without opening the agent", async () => {
    const methods: string[] = [];
    const callGatewayMock = vi.fn(async (options: CallGatewayCliOptions): Promise<unknown> => {
      methods.push(options.method);
      if (options.method === "openclaw.setup.detect") {
    ...
    expect(prompter.outro).toHaveBeenCalledWith("HSMA setup paused.");
    expect(runTui).not.toHaveBeenCalled();
  });
});
