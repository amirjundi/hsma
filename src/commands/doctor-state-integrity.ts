  ...
  switch (issue.kind) {
    ...
    case "missing-state-dir":
      return {
        ...
        path: issue.path,
        fixHint: "Run `hsma doctor --fix` to create the state directory.",
      };
    case "state-dir-not-writable":
      return {
        ...
        path: issue.path,
        fixHint: "Run `hsma doctor --fix` to repair state directory permissions.",
      };
    case "state-dir-too-open":
      return {
        ...
        path: issue.path,
        fixHint: "Run `hsma doctor --fix` to tighten state directory permissions.",
      };
    case "config-file-too-open":
      return {
        ...
        path: issue.path,
        fixHint: "Run `hsma doctor --fix` to tighten config file permissions.",
      };
    case "missing-runtime-dir":
      return {
        ...
        path: issue.path,
        fixHint: "Run `hsma doctor --fix` to create missing runtime state directories.",
      };
    case "runtime-dir-not-writable":
      return {
        ...
        path: issue.path,
        fixHint: "Run `hsma doctor --fix` to repair runtime state directory permissions.",
      };
  }
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        `  Set locally: OPENCLAW_STATE_DIR=~/.openclaw ${formatCliCommand("hsma doctor")}`,
      ].join("\n"),
    );
  }
  if (linuxSdBackedStateDir) {
      ...
      if (missing.length > 0) {
        warnings.push(
          [
            ...
            `  Verify sessions in store: ${formatCliCommand(`hsma sessions --store "${sqliteStorePath}"`)}`,
            `  Preview cleanup impact: ${formatCliCommand(`hsma sessions cleanup --store "${sqliteStorePath}" --dry-run --fix-missing`)}`,
            `  Prune missing entries: ${formatCliCommand(`hsma sessions cleanup --store "${sqliteStorePath}" --enforce --fix-missing`)}`,
          ].join("\n"),
        );
      }
      ...
      if (wedgedSubagentSessions.length > 0) {
        ...
        warnings.push(
          [
            ...
            "  HSMA will not auto-resume these child sessions on restart; reconcile their task records instead.",
            `  Examples: ${wedgedSubagentSessions
              .slice(0, 3)
              .map(({ key }) => key)
              .join(", ")}`,
            `  Fix: ${formatCliCommand("hsma tasks maintenance --apply")}`,
          ].join("\n"),
        );
...
export function collectWorkspaceBackupTip(workspaceDir: string): string | null {
  ...
  return "- Tip: back up the agent workspace in a private git repo; keep ~/.hsma out of git (credentials, sessions). Details: /concepts/agent-workspace#git-backup-recommended";
}
