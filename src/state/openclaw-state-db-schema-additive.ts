...
function secretStoreSchemaSql(): string {
  ...
  if (!hasBoundedSchema) {
    throw new Error("HSMA secret store schema marker is missing.");
  }
...
export function ensureMcpOAuthPendingSchema(database: DatabaseSync): void {
  ...
  if (start < 0 || endMarkerStart < start) {
    throw new Error("HSMA MCP OAuth pending schema marker is missing.");
  }
  database.exec(
...
export function ensureDevicePairingJoinCodeSchema(database: DatabaseSync): void {
  ...
  if (start < 0 || endMarkerStart < start) {
    throw new Error("HSMA device pairing join-code schema marker is missing.");
  }
  database.exec(
...
export function ensureConfigRevisionKeySchema(database: DatabaseSync): void {
  ...
  if (start < 0 || endMarkerStart < start) {
    throw new Error("HSMA config revision key schema marker is missing.");
  }
  database.exec(
