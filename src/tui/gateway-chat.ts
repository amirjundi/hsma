...
export class GatewayChatClient implements TuiBackend {
  ...
  private notifyConnectError(error: Error) {
    ...
    if (
      ...
      error.message = [
        error.message,
        "Pairing request sent. Approve it in that gateway's Control UI (Settings -> Devices), or run `hsma devices approve --latest` on the gateway host, then retry.",
      ].join("\n");
    }
...
async function resolveGatewayConnection(
  ...
  const bootstrap = await resolveGatewayClientBootstrap({
    ...
    overrideAuthErrorHint:
      "Fix: pass --token or --password once to request pairing, approve it in that gateway's Control UI (Settings -> Devices), then retry with the same credential so HSMA can store the device token.",
