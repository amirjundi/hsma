...
describe("tui command handlers", () => {
  ...
  it("returns to HSMA with an optional request", async () => {
    const { handleCommand, addSystem, requestExit, sendChat } = createHarness();

    await handleCommand("/openclaw restart gateway");

    expect(sendChat).not.toHaveBeenCalled();
    expect(addSystem).toHaveBeenCalledWith("returning to HSMA with request: restart gateway");
  ...
  it("leaves a HSMA breadcrumb after switching agents", async () => {
