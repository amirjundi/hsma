    ...
    try {
      ...
      if (isConfigValidationFailedError(error)) {
        ...
        note(
          [
            ...
            ...issueLines,
            `${unpersistedLine} Fix the value(s) above in ${shortenHomePath(ctx.configPath)} by hand, then rerun "hsma doctor --fix".`,
          ].join("\n"),
          "Doctor warnings",
        );
      ...
      note(
        [
          ...
          'Resolve the reported Gateway or cron-store condition, then rerun "hsma doctor --fix".',
        ].join("\n"),
        "Doctor warnings",
      );
...
export async function collectWriteConfigHealthFindings(
  ...
  if (resolveIsNixMode(process.env)) {
    findings.push({
      ...
      severity: "warning",
      message: "Doctor config writes are disabled because HSMA is running in Nix mode.",
