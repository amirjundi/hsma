...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[hsma proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[hsma proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`hsma proxy coverage\` for details.`,
  );
}
