  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your HSMA state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your HSMA state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
  ...
  return [
    {
      ...
      remediation: "Run `hsma doctor --fix` to repair the authored agent roster.",
    },
  ];
}
  ...
  findings.push({
    ...
    detail:
      ...
      `\nMCP server registries visible to the gateway configuration/state:\n${sources
        ...
        .join("\n")}\n` +
      "agents.*.skills filters HSMA skill visibility and snapshots; it is not a shell-time authorization boundary. " +
