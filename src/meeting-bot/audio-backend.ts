...
const PIPEWIRE_MEETING_AUDIO_DEVICE_LABEL = "HSMA Meeting Audio";
...
export async function ensureMeetingAudioBackend(params: {
  ...
  if (!pulseListContains(sinks.stdout ?? "", PIPEWIRE_SINK_NAME)) {
    ...
    if (loaded.code !== 0) {
      ...
      if (!pulseListContains(sinks.stdout ?? "", PIPEWIRE_SINK_NAME)) {
        assertCommandSucceeded(loaded, "Could not create the HSMA PipeWire-Pulse sink");
      }
    }
  }
  ...
  if (!pulseListContains(sources.stdout ?? "", PIPEWIRE_SOURCE_NAME)) {
    ...
    if (loaded.code !== 0) {
      ...
      if (!pulseListContains(sources.stdout ?? "", PIPEWIRE_SOURCE_NAME)) {
        assertCommandSucceeded(loaded, "Could not create the HSMA PipeWire-Pulse source");
      }
    }
  }
  sinks = await params.run(["pactl", "list", "short", "sinks"], params.timeoutMs);
  ...
  if (
    ...
    throw new Error("HSMA PipeWire-Pulse sink or monitor source was not created.");
  }
}
