...
function resolvePackageRoot(packageRoot: string | undefined): string {
  ...
  if (!resolved) {
    throw new Error("Unable to locate the running HSMA package root for worker bundling");
  }
  return resolved;
}
...
async function verifyPublishedNpmRelease(params: {
  ...
  try {
    const published = parseNpmPackageIdentity(
      unwrapNpmJsonEntry(
        await runNpmProofCommand({
          ...
          cwd: temporaryRoot,
          failureMessage: `HSMA ${params.version} is not published; use the worker bundle install`,
          runCommand,
        }),
      ),
    );
    ...
    const packedValue = await runNpmProofCommand({
      ...
      cwd: temporaryRoot,
      failureMessage: "Unable to verify the installed HSMA package; use the worker bundle install",
      runCommand,
    });
    ...
    try {
      ...
      throw new Error("Unable to verify the installed HSMA package; use the worker bundle install");
    }
    if (
      ...
      throw new Error(
        `Installed HSMA ${params.version} does not match the published package; use the worker bundle install`,
      );
    }
    ...
    if (packedBundle.bundleHash !== params.bundleHash) {
      throw new Error(
        `Published HSMA ${params.version} does not match the prepared worker bundle; use the worker bundle install`,
      );
    }
...
async function prepareWorkerBundle(
  ...
  if (!openclawVersion) {
    throw new Error("Worker bundle requires a non-empty HSMA version");
  }
