...
export const GATEWAY_STALE_INSTALL_CLOSE_REASON =
  "gateway install changed; run: hsma gateway restart";
...
export function classifyGatewayStaleInstall(error: unknown): GatewayStaleInstall | null {
  ...
  const restartCommand = formatCliCommand("hsma gateway restart");
  return {
    error: errorShape(
      ErrorCodes.UNAVAILABLE,
      `The running Gateway can no longer load part of its HSMA installation. The installation may have changed while the Gateway was running. Restart it with: ${restartCommand}`,
