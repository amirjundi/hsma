    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["hsma nodes status", "List known nodes with live status."],
          ["hsma nodes pending", "Show pending node pairing requests."],
          ["hsma nodes remove --node <id|name|ip>", "Remove a stale paired node entry."],
          [
            'hsma nodes invoke --node <id> --command system.which --params \'{"bins":["uname"]}\'',
            "Invoke a node command directly.",
          ],
          ["hsma nodes camera snap --node <id>", "Capture a photo from a node camera."],
...
function shouldRegisterNodesPluginCommands(nodes: Command, argv: readonly string[]): boolean {
  ...
  if (!requestedSubcommand) {
    // Bare `hsma nodes` listing should still surface plugin-provided subcommands.
    return true;
  }
