...
export function registerWebhooksCli(program: Command) {
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + HSMA hooks")
    ...
    .option("--hook-url <url>", "HSMA hook URL")
    .option("--hook-token <token>", "HSMA hook token")
...
function parseGmailSetupOptions(raw: Record<string, unknown>): GmailSetupOptions {
  ...
  if (!account) {
    throw new Error(
      `--account is required. Example: ${formatCliCommand("hsma webhooks gmail setup --account default")}.`,
    );
  }
