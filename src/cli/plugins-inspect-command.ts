// `hsma plugins inspect`: renders plugin registry shape, capabilities, policy, diagnostics, and install records.
...
/** Options accepted by `hsma plugins inspect`. */
...
export async function runPluginsInspectCommand(
  ...
  if (!targetPlugin) {
    if (id === "skill-workshop") {
      ...
      const lines = [
        "Skill Workshop is built into HSMA, not a plugin; configure it under skills.workshop.",
      ];
  ...
  if (!inspect) {
    failPluginInspect(
      formatMissingPluginMessage({ id, listCommand: "hsma plugins list --json" }),
      opts.json,
    );
    return;
  }
