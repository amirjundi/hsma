  ...
  return {
    ...
    warnings: [
      ...(pluginUpdate.warnings ?? []),
      {
        ...
        guidance: ["Run `hsma update repair` to retry post-update plugin repair."],
      },
    ],
  };
}
...
export async function runUpdateFinalizationDoctorInFreshProcess(params: {
  ...
  if (!entryPath) {
    throw new Error("Updated HSMA entrypoint not found for post-plugin doctor");
  }
  const args = [
    entryPath,
    "doctor",
  ...
  if (!entryPath) {
    return {
      pluginUpdate: createPostPluginDoctorExecutionFailure(
        params.pluginUpdate,
        "Updated HSMA entrypoint not found for post-plugin doctor",
      ),
      configValid: false,
    };
  }
