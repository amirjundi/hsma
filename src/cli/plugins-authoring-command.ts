...
export function buildToolPluginPackageManifest(params: {
  ...
  const openclaw =
    ...
    !Array.isArray(params.packageManifest.openclaw)
      ? { ...(params.packageManifest.hsma as JsonObject) }
      : {};
...
export function validateToolPluginProject(params: {
  ...
  if (!jsonSchemaValuesEqual(params.manifest, expectedManifest)) {
    errors.push("openclaw.plugin.json generated metadata is stale. Run hsma plugins build.");
  }
...
export async function runPluginsBuildCommand(opts: PluginsBuildOptions): Promise<void> {
  ...
  if (opts.check) {
    ...
    if (
      ...
      defaultRuntime.error("Generated plugin metadata is out of date. Run hsma plugins build.");
...
function writeToolPluginScaffold(params: { rootDir: string; id: string; name: string }): void {
  const packageManifest = {
    ...
    scripts: {
      ...
      "plugin:build": "npm run build && hsma plugins build --entry ./dist/index.js",
      "plugin:validate": "npm run build && hsma plugins validate --entry ./dist/index.js",
  ...
  const description = `Add ${params.name} tools to HSMA.`;
...
function writeProviderPluginScaffold(params: { rootDir: string; id: string; name: string }): void {
  ...
  const description = `Add ${params.name} models to HSMA.`;
  const packageManifest = {
    name: packageName,
    version: "0.1.0",
    description: `HSMA provider plugin for ${params.name}.`,
    type: "module",
    scripts: {
      build: "tsc -p tsconfig.json",
      test: "vitest run --config ./vitest.config.ts",
...
HSMA provider plugin for ${params.name}.
