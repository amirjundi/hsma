...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["hsma security audit", "Run a local security audit."],
          [
            "hsma security audit --deep",
            "Include best-effort live Gateway probes and plugin-owned security audit collectors.",
          ],
          ["hsma security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "hsma security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          [
            "hsma security audit --auth password --password <password>",
            "Audit a runtime-only password-mode Gateway secret.",
          ],
          ["hsma security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["hsma security audit --json", "Output machine-readable JSON."],
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("HSMA security audit"));
      ...
      lines.push(muted(`Run deeper: ${formatCliCommand("hsma security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("hsma security audit --fix")}`));
        if (!fixResult) {
