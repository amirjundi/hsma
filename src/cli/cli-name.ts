...
// HSMA ships as `hsma`. The entry file is still openclaw.mjs -- renaming it would
...
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(hsma|openclaw)\b/;

/** Resolve the displayed CLI binary name from argv, falling back to `hsma`. */
