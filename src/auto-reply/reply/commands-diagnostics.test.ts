    ...
    if (ctx.diagnosticsPreviewOnly) {
      return {
        text: [
          ...
          "The completed diagnostics reply will list the HSMA session ids and Codex thread ids that were sent.",
...
describe("diagnostics command", () => {
  ...
  it("omits the Codex section for ordinary sessions without Codex targets", async () => {
    registerHostTrustedReservedCommandForTest({
      ...
      handler: vi.fn(async () => ({
        text: [
          "No Codex thread is attached to this HSMA session yet.",
  ...
  it("routes group diagnostics confirmations privately", async () => {
    const commandHandler = vi.fn(async () => ({
      text: [
        "Codex diagnostics sent to OpenAI servers:",
        "- channel whatsapp, HSMA session session-1, Codex thread codex-thread-1",
      ].join("\n"),
    }));
    registerHostTrustedReservedCommandForTest({
      name: "codex",
