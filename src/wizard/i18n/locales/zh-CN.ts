...
export const zh_CN = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密码存入 HSMA 配置",
      ...
      tokenStoreProvisioned:
        "已生成 Gateway 令牌并以 {name} 存入 HSMA 密钥存储。配置中只保留引用；可用 `hsma secrets store list` 查看。",
    ...
    migration: {
      ...
      appliedTitle: "迁移已应用",
      cancelled: "迁移已取消",
      complete: "迁移完成。下一步运行 `hsma doctor`。",
    ...
    memoryImport: {
      ...
      confirm: "将这些 memory 导入你的 HSMA 工作区？",
    ...
    remote: {
      ...
      plaintextPasswordHint: "直接把密码存入 HSMA 配置",
      plaintextPasswordLabel: "现在输入密码",
      plaintextTokenHint: "直接把令牌存入 HSMA 配置",
    ...
    guided: {
      ...
      custodianIntro: "你好 — 我是 HSMA，负责维护这套系统。我们开始设置吧。",
      ...
      appliedTitle: "推理已就绪",
      complete: "HSMA 已准备就绪。",
      completeWithoutAi: "HSMA 设置已保存。连接 AI 后再打开聊天。",
      detected: "AI 检测完成。",
      ...
      escapeHatches: "如需完整的分步向导，请运行 `hsma onboard --classic`。",
      ...
      intro: "连接你的 AI",
      invalidConfigDetails: "HSMA 配置 {path} 无效：\n{issues}",
      invalidConfigRepair:
        "未进行任何更改。请运行 {fixCommand} 修复，再运行 {inspectCommand} 检查，然后重试 onboarding。",
      invalidConfigUnknown: "- 无法解析配置。",
      manualChoice: "你想如何连接 AI？",
      nextSteps:
        "工作区：{workspace}\n添加频道：`hsma channels add`\n更喜欢聊天？运行 `hsma setup`，然后说 `connect telegram`（或 `connect slack`）。\n打开 dashboard：`hsma dashboard`\n稍后聊天：`openclaw`",
      nextStepsWithoutAi:
        "工作区：{workspace}\n稍后添加 AI：重新运行 `hsma onboard`\n连接 AI 后添加频道：`hsma channels add`\n打开 dashboard：`hsma dashboard`",
      nextStepsTitle: "下一步",
      noInferenceOptions:
      ...
      ttyRequired:
        "Onboarding 需要交互式 TTY。自动化请使用 `hsma onboard --non-interactive --accept-risk ...`。",
      welcomeTitle: "设置选项",
    ...
    security: {
      ...
      attribution: "HSMA 是由 HSMA 基金会（非营利组织）开发的开源助手，会与你一同学习成长。",
      baselineDmSessions:
      ...
      baselineSharedInbox:
        "共享收件箱：使用独立的 gateway 和凭据；独立 OS 用户或主机可提供更强隔离。",
      baselineStrongModel: "对启用了工具或使用共享/公开收件箱的 bot，使用可用的最强模型。",
      confirm: "我理解 HSMA 默认面向个人使用；共享/多用户使用需要加固。继续？",
      hardeningRequired: "如果你不熟悉访问控制和安全加固，请在获得帮助前不要运行 HSMA。",
      learnMore: "了解更多请访问",
      notMultitenant: "默认情况下，HSMA 并非旨在安全地隔离多个用户。",
      personalAgent: "默认情况下，HSMA 是供一位操作员使用的个人 agent。",
    ...
    telemetry: {
      ...
      decline: "不用，谢谢",
      description:
        "在每日更新检查中分享你使用的功能（频道、提供商、插件数量）。绝不包含消息或标识符。查看实际发送的内容：`hsma telemetry show`。随时更改：`hsma telemetry on|off`。",
      title: "帮助 HSMA 变得更好？",
    },
    skills: {
      configure: "现在配置技能？（推荐）",
      ...
      manualPrereqsDoctorHint: "请先安装这些工具，然后运行 `hsma doctor` 检查技能和要求。",
    ...
    clickclack: {
      ...
      helpCreateToken: "在 ClickClack 中：工作区设置 → Integrations → HSMA → 创建 bot → 复制 token",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 运行：./occ talk:bot:install "HSMA" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要发送者/所有者号码，以便 HSMA 将你加入允许列表。",
      ...
      separatePhoneLabel: "HSMA 专用独立手机",
    ...
    irc: {
      ...
      nextStatusCommand: "命令：hsma channels status --probe",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "这是 HSMA 用来向 Chat 发送回复的 URL。",
      ...
      publicWebhookUrlScope: "仅公开此 webhook 路由；HSMA 使用它提供短期附件下载。",
    ...
    signal: {
      ...
      multipleEntries: "多个条目请用逗号分隔。",
      nextLinkDevice: '绑定设备：signal-cli link -n "HSMA"',
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "将 Synology NAS Chat 连接到 HSMA，并启用完整 agent 能力。",
    ...
    search: {
      ...
      configured: "已配置",
      configureLaterHint: "稍后可用 hsma configure --section web 配置",
      docsLine: "文档：{url}",
    ...
    finalize: {
      ...
      dashboardCopyPaste: "在本机浏览器中复制/粘贴这个 URL 来控制 HSMA。",
      dashboardOpened: "已在浏览器中打开。保留该标签页以控制 HSMA。",
      ...
      optionalApps: "可选应用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 链接控制 HSMA。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已打开；保留该标签页以控制 HSMA。",
      ...
      containerSystemdUnavailable:
        "此容器内没有 systemd 用户服务。HSMA 只会跳过后台服务安装；请以前台方式运行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
      ...
      webUiUrl: "Web UI：{url}",
      whatNow: '下一步：https://docs.openclaw.ai/showcase（"What People Are Building"）。',
      whatNowTitle: "下一步",
