...
export const zh_TW = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密碼存入 HSMA 設定",
      ...
      tokenStoreProvisioned:
        "已產生 Gateway 權杖並以 {name} 存入 HSMA 祕密儲存。設定中只保留參照；可用 `hsma secrets store list` 檢視。",
    ...
    migration: {
      ...
      appliedTitle: "遷移已套用",
      cancelled: "遷移已取消",
      complete: "遷移完成。下一步執行 `hsma doctor`。",
    ...
    memoryImport: {
      ...
      confirm: "將這些 memory 匯入你的 HSMA 工作區？",
    ...
    remote: {
      ...
      plaintextPasswordHint: "直接把密碼存入 HSMA 設定",
      plaintextPasswordLabel: "現在輸入密碼",
      plaintextTokenHint: "直接把權杖存入 HSMA 設定",
    ...
    guided: {
      ...
      custodianIntro: "你好 — 我是 HSMA，負責維護這套系統。我們開始設定吧。",
      ...
      appliedTitle: "推理已就緒",
      complete: "HSMA 已準備就緒。",
      completeWithoutAi: "HSMA 設定已儲存。連接 AI 後再開啟聊天。",
      detected: "AI 偵測完成。",
      ...
      escapeHatches: "如需完整的分步精靈，請執行 `hsma onboard --classic`。",
      ...
      intro: "連接你的 AI",
      invalidConfigDetails: "HSMA 設定 {path} 無效：\n{issues}",
      invalidConfigRepair:
        "未進行任何變更。請執行 {fixCommand} 修復，再執行 {inspectCommand} 檢查，然後重試 onboarding。",
      invalidConfigUnknown: "- 無法解析設定。",
      manualChoice: "你想如何連接 AI？",
      nextSteps:
        "工作區：{workspace}\n新增頻道：`hsma channels add`\n偏好聊天？執行 `hsma setup`，然後說 `connect telegram`（或 `connect slack`）。\n開啟 dashboard：`hsma dashboard`\n稍後聊天：`openclaw`",
      nextStepsWithoutAi:
        "工作區：{workspace}\n稍後新增 AI：重新執行 `hsma onboard`\n連接 AI 後新增頻道：`hsma channels add`\n開啟 dashboard：`hsma dashboard`",
      nextStepsTitle: "下一步",
      noInferenceOptions:
      ...
      ttyRequired:
        "Onboarding 需要互動式 TTY。自動化請使用 `hsma onboard --non-interactive --accept-risk ...`。",
      welcomeTitle: "設定選項",
    ...
    security: {
      ...
      attribution: "HSMA 是由 HSMA 基金會（非營利組織）開發的開源助手，會與你一同學習成長。",
      baselineDmSessions:
      ...
      confirm: "我理解 HSMA 預設面向個人使用；共享/多使用者使用需要加固。繼續？",
      hardeningRequired: "如果你不熟悉存取控制和安全加固，請在獲得協助前不要執行 HSMA。",
      learnMore: "了解更多請參閱",
      notMultitenant: "預設情況下，HSMA 並非旨在安全地隔離多個使用者。",
      personalAgent: "預設情況下，HSMA 是供一位操作者使用的個人 agent。",
    ...
    telemetry: {
      ...
      decline: "不用，謝謝",
      description:
        "在每日更新檢查中分享你使用的功能（頻道、提供者、插件數量）。絕不包含訊息或識別碼。查看實際傳送的內容：`hsma telemetry show`。隨時變更：`hsma telemetry on|off`。",
      title: "幫助 HSMA 變得更好？",
    },
    skills: {
      configure: "現在設定技能？（建議）",
      ...
      manualPrereqsDoctorHint: "請先安裝這些工具，然後執行 `hsma doctor` 檢查技能和需求。",
    ...
    clickclack: {
      ...
      helpCreateToken: "在 ClickClack 中：工作區設定 → Integrations → HSMA → 建立 bot → 複製 token",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 執行：./occ talk:bot:install "HSMA" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要傳送者/擁有者號碼，以便 HSMA 將你加入允許清單。",
      ...
      separatePhoneLabel: "HSMA 專用獨立手機",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "這是 HSMA 用來向 Chat 傳送回覆的 URL。",
      ...
      publicWebhookUrlScope: "僅公開此 webhook 路由；HSMA 使用它提供短期附件下載。",
    ...
    signal: {
      ...
      multipleEntries: "多個項目請用逗號分隔。",
      nextLinkDevice: '綁定裝置：signal-cli link -n "HSMA"',
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "將 Synology NAS Chat 連接到 HSMA，並啟用完整 agent 能力。",
    ...
    search: {
      ...
      configured: "已設定",
      configureLaterHint: "稍後可用 hsma configure --section web 設定",
      docsLine: "文件：{url}",
    ...
    finalize: {
      ...
      dashboardCopyPaste: "在本機瀏覽器中複製/貼上這個 URL 來控制 HSMA。",
      dashboardOpened: "已在瀏覽器中開啟。保留該分頁以控制 HSMA。",
      ...
      optionalApps: "可選應用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 連結控制 HSMA。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已開啟；保留該分頁以控制 HSMA。",
      ...
      containerSystemdUnavailable:
        "此容器內沒有 systemd 使用者服務。HSMA 只會略過背景服務安裝；請以前景方式執行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
