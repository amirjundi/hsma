...
// update.
//
// `runGatewayUpdate` (git mode) runs `hsma doctor --fix` with
...
// `shouldDeferConfiguredPluginInstallRepair`). The `hsma update` CLI resumes
...
// binary's hidden `hsma update finalize` entrypoint — the designed
...
// re-enters the full `hsma update` CLI. Re-run convergence on no-op retries:
