...
describe("ports helpers", () => {
  ...
  it("prints an HSMA-specific hint when port details look like another HSMA instance", async () => {
    const runtime = {
      error: vi.fn(),
      log: vi.fn(),
      exit: vi.fn() as unknown as (code: number) => never,
    ...
    const messages = runtime.error.mock.calls.map((call) => stripAnsi(String(call[0] ?? "")));
    expect(messages.join("\n")).toContain("another HSMA instance is already running");
  });
});
...
describe("inspectPortUsage on Windows", () => {
  ...
  it("uses PowerShell process command lines to classify HSMA listeners", async () => {
    mockWindowsCommands({
      netstat: commandOutput("  TCP    127.0.0.1:18789    0.0.0.0:0    LISTENING    4242\r\n"),
      tasklist: commandOutput('"node.exe","4242","Console","1","10,000 K"\r\n'),
      powershell: commandOutput(
