...
function ruleRow(params?: {
  ...
  return {
    DisplayName: params?.displayName ?? "HSMA Gateway",
...
describe("Windows Gateway firewall diagnostics", () => {
  ...
  it.each([
    [
      ...
      "HSMA could not quickly inspect Windows Firewall LAN Gateway policy.",
    ],
    [
      "truncated stdout",
    ...
    [
      "malformed JSON",
      async () => ({ code: 0, stdout: "{" }),
      "HSMA could not parse Windows Firewall LAN Gateway policy.",
    ],
    [
      "a runner exception",
      async () => {
